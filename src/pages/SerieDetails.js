...
function SerieDetails({ API_KEY, API_URL }) {
  ...
  function fetchSerie() {
      fetch(`${API_URL}/tv/${id}?api_key=${API_KEY}&language=en-US&append_to_response=videos`)
      .then(res => res.json())
      .then(data => {
        setTimeout(() => {
          setSerie(data)
  ...
  function fetchCast() {
    fetch(`${API_URL}/tv/${id}/credits?api_key=${API_KEY}&language=en-US`)
      .then(res => res.json())
  ...
  const backdropUrl = getMoviePoster(serie?.backdrop_path, "w1280")
  const posterUrl = getMoviePoster(serie?.poster_path, 'w500')
              ...
              >   
                ...
                <div className='movie-details-content background-img-shadow relative flex flex-col md:flex-row  md:max-h-[480px] lg:max-h-[540px] 2xl:max-h-[630px]'>
                  ...
                  <div className='movie-details-img'>
                      <img src={posterUrl} alt={serie.name} className='w-screen md:max-w-[320px] lg:max-w-[360px] 2xl:max-w-[420px]'/>
                  </div>
                  ...
                  <div className='movie-details-info relative flex flex-col md:w-[440px] lg:w-[550px] 2xl:w-[640px]'>
                    ...
                    <div className='text-center md:text-left'>
                      ...
                      <div className={`${displayTrailer?'':'md:hidden'} relative h-[400px] w-full md:static`}>
                        {serie.videos?<MovieTrailer movie={serie}/>:null}
                      </div>
                      {/* MOVIE CAST */}
